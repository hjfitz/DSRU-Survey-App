...
class SurveyController {
	...
	static mapIDtoParent(question, prev = '') {
		const text = prev ? `${prev} (${question.questionText})` : question.questionText
		const ds = {id: question._id, question: text}
		const subquestions = question.options.filter(opt => opt.question).map(opt => SurveyController.mapIDtoParent(opt.question, question.questionText))
