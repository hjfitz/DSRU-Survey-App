...
let pass = process.env.ADMIN_PASS || 'pass'
const adminUser = process.env.ADMIN_USER || 'admin'

console.log('login info:', {adminUser, pass})

class AuthController {
	static login(req, res) {
		const {user, password} = req.body
		console.log('Attempting to auth user:', {user, password})
		if (user !== adminUser) {
	...
	static handleRequest(req, res, next) {
		if (!req.session.authed && process.env.AUTH_ENABLED === 'true' || !process.env.AUTH_ENABLED) {
