...
const handleImageUrl = require('./handle-images')
const db = require('../db')
...
class SurveyController {
	...
	static async createCSV(surveyID, field, value, res) {
		...
		const fields = responses.map(resp => resp.answers.map(an => an.questionID)).flat()
		const csv = parse(keyval, {fields})
		res.setHeader('content-type', 'text/csv')
		res.send(csv)
	}

	static generateMapping(req, res) {
		const id = req.params.id.replace('.csv', '')
		SurveyController.createCSV(id, 'questionID', 'questionText', res)
	}
	...
	static async editSurvey(req, res) {
		...
		try {
			...
			// update survey
			survey.questions = await Promise.all(questions.map(qu => SurveyController.createQuestion(qu, survey._id)))
			survey.title = title
	...
	static async createSurvey(req, res) {
		...
		try {
			...
			console.log(survey)
			const questions = await Promise.all(rawQuestions.map((raw) => {
				console.log(survey._id)
				return SurveyController.createQuestion(raw, survey._id)
			}))
