...
let curSecret = crypto.randomBytes(128).toString('hex')

// set a new session secret every 2 hours
setInterval(() => {
	curSecret = crypto.randomBytes(128).toString('hex')
...
async function findSurveysAndResponseCount(page) {
	const skip = (page - 1) * 10
	const all = await db.Survey.find({}, null, {sort: {created: -1}}).limit(10).skip(skip)
	...
	const count = await db.Survey.estimatedDocumentCount()
...
buildRouter.get('/all', async (req, res) => {
	...
	const allAndResponseCount = await findSurveysAndResponseCount(page)
